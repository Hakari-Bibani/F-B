...
class Calculator:
    ...
    def create_layout(self):
        ...
        with col2:
            self.t_solution_input = st.text_input(f"**پلەی {self.process_name} گیراوە:**", key="t_solution")
            self.t_solution_unit = st.selectbox("یەکە:", ["Celsius", "Kelvin"], key="t_solution_unit")
            self.t_solvent_input = st.text_input(f"**پلەی {self.process_name} توێنەر:**", key="t_solvent")
...
class FreezingPointCalculator(Calculator):
    def __init__(self):
        self.t_symbol = "Tf"
        self.process_name = "بەستن"
        super().__init__("نزمبونەوەی پلەی بەستن: ژمێرکاری بۆ تواوەی نا ئەلیکترۆلیتی")

class BoilingPointCalculator(Calculator):
    def __init__(self):
        self.t_symbol = "Tb"
        self.process_name = "کوڵان"
