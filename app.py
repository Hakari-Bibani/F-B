...
class BaseCalculator:
    ...
    def show_calculation_step(self, equation, values, result):
        values_str = " = " + " - ".join(f"{v:.4f}" for v in values) if ' - ' in equation else " = " + " / ".join(f"{v:.4f}" for v in values)
    ...
    def calculate(self):
        ...
        inputs['t_solution'] = self.convert_temperature(inputs['t_solution'], st.session_state.t_solution_unit)
        inputs['t_solvent'] = self.convert_temperature(inputs['t_solvent'], st.session_state.t_solvent_unit)
        
        if inputs['mass_solute'] is not None:
            inputs['mass_solute'] = self.convert_mass(inputs['mass_solute'], st.session_state.mass_solute_unit, 'grams')
        if inputs['kg_solvent'] is not None:
            inputs['kg_solvent'] = self.convert_mass(inputs['kg_solvent'], st.session_state.kg_solvent_unit, 'kilograms')
        ...
        calculations = [
            ...
            {
                'param': 'kg_solvent',
                'func': lambda mass: mass / 1000,
                'equation': 'توێنەر-Kg = تواوە-mass × 1000',
                'params': ['mass_solute']
            },
        ]
        ...
        for calc in calculations:
            if self.try_calculate_value(inputs, calc['param'], calc['func'], calc['equation'], calc['params']):
                st.write(f"Calculated {calc['param']} successfully.")

        st.write("-" * 50)

# Example usage
image1, image2 = load_images()
FreezingPointCalculator = BaseCalculator("ژمێرکاری دەرکردنی تەنیشت")
